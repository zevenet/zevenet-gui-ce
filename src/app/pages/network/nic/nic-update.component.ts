...
@Component({
  selector: 'zevenet-network-nic-update',
  templateUrl: './nic-update.component.html',
})
export class NicUpdateComponent implements  OnInit {
  ...
  public getInterface(name, refresh = false): void {
    this.service.getObject('interfaces/nic', this.name)
      .subscribe(
        (data) => {
  ...
  onSubmit() {
    ...
    if (Object.keys(submit).length > 0) {
      ...
      this.service.update('interfaces/nic', this.name, submit)
        .subscribe(
          (data) => {
