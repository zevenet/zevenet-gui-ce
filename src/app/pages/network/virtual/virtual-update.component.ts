...
@Component({
  selector: 'zevenet-network-virtual-update',
  templateUrl: './virtual-update.component.html',
})
export class VirtualUpdateComponent implements  OnInit {
  ...
  getInterface(name, refresh = false): void {
    this.service.getObject('interfaces/virtual', this.name)
      .subscribe(
        (data) => {
  ...
  onSubmit() {
    ...
    if (Object.keys(submit).length > 0) {
      ...
      this.service.update('interfaces/virtual', this.name, submit)
        .subscribe(
          (data) => {
          ...
          () => {
            ...
            this.interfaceValues = this.form.value;
            this.denySubmit = false;
            this.showMessageTranslated('SYSTEM_MESSAGES.network.virtual_interface_updated', 'toast', this.name);
          });
    }
  }
